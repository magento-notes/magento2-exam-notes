...
* Api - Any PHP classes exposed to
                            the API.
* Block - PHP view
...
* [`config.xml`](#h.glpavwxvxuhd) - Adminhtml menus default values[crontab.xml](#h.nzp3v0np709e) - CRON tasks group[], job instance, method, schedule *[cron_groups.xml](#h.nzp3v0np709e) - Grouping
                            / split processes[di.xml](#h.9ofwnl5q9fnf) - preferences,
...
* [`eav_attributes.xml`](#h.wm8oeat9wvdw) - EAV
                            attribute configuration: locked entity attributes (global, unique etc.)email_templates.xml - id label file type
                            module -- view/frontend/email/name.html[events.xml](#h.y2lezqicnwxl) -
...
* [`indexer.xml`](#h.xa0vtkhng9ut) - Create a new
                            indexer class, view_id, title, description[integration.xml](#h.t252zlddzc3r)
...
* [`menu.xml`](#h.gk6g5dbgvggm) - Admin main left
                            menu [module.xml](#h.n1343l3gk3km) - (required) Defines a module: version, sequence[mview.xml](#h.2m5sw7piaq5j) - Indexes, React to MySQLtriggers - scheduled updates, subscribe to
                            table changes, indexer modelpage_types.xml
...
search_request.xml - Elastic search
                            ...
                            -> invalidate customer sections[system.xml](#h.op69p944prs) - adminhtml menu
                            system configvalidation.xml - Regex validation entity, rules, constraints ->
                            classview.xml - vars by
                            module[webapi.xml](#h.2lcvcwr56aq) - Create new API
...
* Default:
  ...
  * Static assets created dynamically - copied!
  * Changes not visible. cached
* Developer:
  * Exceptions displayed, not logged exception thrown if bad event subscriber. var/report detailed.
  * Static assets created dynamically.
  * Changes visible immediately error handler - throws exception instead of logging (notice etc.).
* Production - max speed, no errors, no file generation: admin can't enable/disable cache types
  * errors logged, not displayed
  * Static assets not created dynamically, must be deployed not need for www-data to write, pub/static can be read-only
